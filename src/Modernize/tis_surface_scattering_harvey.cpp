	...
	if (integration_brdf) {
		...
		for (int i_phi = 0; i_phi < nb_points; i_phi++) {
			...
			for (int i = 0; i < nb_points; ++i) {
				...
				fp hv = harvey_brdf(-scattering_angle_wrt_incoming + theta0, theta0, b_surface_roughness, s_6nm[channel - 1],
									l_6nm[channel - 1], m_6nm[channel - 1], n_6nm[channel - 1]);
...
void tis_surface_scattering_harvey(fp* TIS, int channel, Matrix1D& theta0, fp s_rough_mirror, bool integration_brdf)
{
	...
	if (integration_brdf) {
		...
		int nb_i_angles = theta0.m_data.size();
